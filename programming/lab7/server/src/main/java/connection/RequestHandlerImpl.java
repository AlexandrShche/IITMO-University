...
public class RequestHandlerImpl implements RequestHandler {
    ...
    public Response handleRequest(Request request) {
        ...
        String user;
        if(curRequest.getAuth() != null) {
            user = curRequest.getAuth().getLogin();
        } else {
            user = "null user";
        }
        ...
        command = curRequest.getCommand();
        log.Logback.getLogger().info("Command was decrypted");
        String result;
        try {
            result = commandExecutor.executeCommand(command, curRequest.getAuth());
        } catch (Exception e){
            e.printStackTrace();
